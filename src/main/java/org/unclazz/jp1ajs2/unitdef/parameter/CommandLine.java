 ...
 * teはUNIXジョブにおける実行ファイル名、scはPCジョブにおける実行ファイル名である。
 * いずれも実際には「ファイル名」という域を超えておりコマンドライン文字列の指定が可能となっている。
 * teの場合上限は1023文字、scの場合上限は511文字であるが、このような文脈依存の制約についてはこのオブジェクトでは関知しない。
 */
public final class CommandLine {
	/**
	 * インスタンスを返す.
	 * 空白が含まれている場合、コマンドと引数、引数と引数を区切る文字として認識される。
	 * ダブルクオテーションにより、空白文字を含む文字列を単一のコマンドもしくは引数として示すことができる。
	 * またダブルクオテーションで囲われた文字列内でダブルクオテーションを使用する場合は直前にバックスラッシュを付与してエスケープする。
	 * バックスラッシュそのものはバックスラッシュによりエスケープする。
	 * 
