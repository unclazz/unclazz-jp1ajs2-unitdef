 ...
 */
public final class TupleIterableQuery<T> 
extends IterableQuerySupport<T, Tuple> {
	...
	TupleIterableQuery(final IterableQuery<T, ParameterValue> baseQuery, final List<Predicate<Tuple>> preds) {
	...
	TupleIterableQuery(final IterableQuery<T, ParameterValue> baseQuery) {
	...
	public TupleIterableQuery<T> and(final Predicate<Tuple> pred) {
		...
		return new TupleIterableQuery<T>(this.baseQuery, newPreds);
	}
